...
function authenticate_user($email, $password) {
    ...
    if (!$result) die("Database access failed: " . pg_last_error());
...
function update_user($data) {
    ...
    $query = 
        "UPDATE $user_table SET 
         ...
         WHERE id = $id";
    if (pg_query($connection, $query)) return true;
    error_log("Failed to update user: " . pg_last_error());
    return false;
}
...
function get_all_users(){
    ...
    $users_query = "SELECT *
                    FROM $users_table";
    $query_result = pg_query( $connection,  $users_query);
    $rows = array();
...
function user_exists($email) {
    ...
    $s_email = functions\sanitize_string($email);
    $query = "SELECT * FROM ".USER_TABLE." WHERE email_address='$s_email'";
    if (pg_num_rows(pg_query( $connection, $query)))
        return true;
    else
        return false;
}
