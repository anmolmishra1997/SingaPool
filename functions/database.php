...
(
    trip_id INT CHECK(trip_id >= 0) NOT NULL,
...
function add_trip($data)
{
    ...
    if (!pg_query($connection, $query)) {
        error_log("Failed to add place: " . pg_last_error());
        return NULL;
    }
...
function add_place($data)
{
    ...
    if (!pg_query($connection, $query)) {
        echo "Failed to add place: " . pg_last_error();
        return NULL;
    }
    $insert_query = pg_query("SELECT lastval();");
    $insert_row = pg_fetch_row($insert_query);
...
function get_rides_for($user_id) {
    ...
    $trips_result = pg_query($connection, $trips_query);
    $trips = array();
    if ($trips_result) {
        for ($i = 0; $i < pg_num_rows($trips_result); ++$i) {
            $row = pg_fetch_assoc($trips_result);
...
function get_trips_near_on($route, $departure=NULL, $user_id=NULL) {
    ...
    $result = pg_query($connection, $search_query);
    $rows = array();
...
function update_spots_taken($trip_id) {
    ...
    $spots_taken = pg_num_rows(pg_query($connection, $spots_query));
    ...
    if (pg_query($connection, $update_query))
        return $spots_taken;
    else
        return -1;
}
...
function delete_trip($trip_id){
    ...
    if ($trip){
        ...
        $delete_trip = pg_query($connection, "DELETE FROM $trip_table WHERE id=$trip_id");
        $delete_place_1 = pg_query($connection, "DELETE FROM $place_table WHERE id= $origin ");
        $delete_place_2 = pg_query($connection, "DELETE FROM $place_table WHERE id=$destination");
