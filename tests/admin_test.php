...
function delete_user1($user_id){
    ...
    $trip_table = 'trip';
    $s_user_id= $user_id;
    //$users_table_delete = pg_query($connection, "DELETE FROM $user_table WHERE id = $s_user_id");
    $users_trips = pg_query($connection, "SELECT * FROM $trip_table WHERE driver_id=$s_user_id");
    $num_rows = pg_num_rows($users_trips);
    //if ($users_table_delete){
        if ($users_trips ){
            for ($i = 0; $i < $num_rows ; ++$i) {
                $row = pg_fetch_assoc($users_trips);
                var_dump($row);
                var_dump($row['id']);
